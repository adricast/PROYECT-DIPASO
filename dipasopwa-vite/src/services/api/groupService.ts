...
import { v4 as uuidv4 } from 'uuid'; // Importamos v4 para generar el ID único
 ...
 * @returns El objeto de grupo con los nombres de propiedades corregidos y el ID local.
 */
function mapGroupFromApi(apiGroup: any): Group {
    return {
        id: uuidv4(), // ✅ CORRECCIÓN CLAVE: Generar un ID local único para el objeto.
        groupId: apiGroup.user_group_id,
        groupName: apiGroup.group_name,
        description: apiGroup.description,
        users: [],
        syncStatus: "backend" as GroupSyncStatus, // Estado inicial para grupos del backend.
...
async function handleApiError(group: Group, error: unknown) {
    ...
    if (isNotFound) {
        ...
        // ✅ CORRECCIÓN: Usar el 'id' local para eliminar, no el groupId o tempId.
        if (group.id) {
...
export async function createGroup(group: Omit<Group, "id" | "syncStatus">): Promise<Group> {
    // ✅ CORRECCIÓN CLAVE: Generar 'id' y 'tempId' dentro de la función.
    const newGroup = {
        ...group,
    ...
    for (const group of allGroups) {
        ...
        try {
            ...
            if (originalStatus === "pending") {
                ...
                // ✅ CORRECCIÓN: Usar el 'id' local para eliminar.
                if (group.id) {
                    await groupRepo.deleteGroup(group.id);
                }
