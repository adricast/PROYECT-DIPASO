import json
import os
from models.userjson_model import UserJSON  # <- tu modelo de usuario
...
def read_users():
    ...
    try:
        with open(USERS_FILE, "r") as f:
            ...
            return [UserJSON(**u) for u in users_data]
...
class AuthService:
    def login(self, username, password):
        ...
        if user:
            ...
            write_auth({"token": token, "user": user.to_dict()})  # <-- importante usar to_dict()
            return True, user.to_dict()                           # <-- y aquí también
